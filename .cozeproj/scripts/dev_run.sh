...
start_service() {
    ...
    if [ "${COZE_DEVBOX_ENV}" = "prod" ]; then
        # 生产环境：只启动后端服务，后端同时提供前端静态文件
        echo "🚀 Production mode detected, starting backend server with static files..."

        # 启动后端（同时提供前端静态文件）
        cd server
        # 设置端口为 5000，让后端同时提供 API 和前端静态文件
        export SERVER_PORT=${DEPLOY_RUN_PORT}
        npm run start:prod &
        local server_pid=$!

        echo "${server_pid}" > "${PID_FILE}"
        echo "📝 Backend server (with static files) started with PID: ${server_pid} on port ${DEPLOY_RUN_PORT}"
...
echo "Starting HTTP services on port ${DEPLOY_RUN_PORT}..."
start_service
