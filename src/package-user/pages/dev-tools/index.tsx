    ...
    <View className="min-h-screen bg-slate-900 px-6 py-8">
          ...
          <View className="space-y-3">
            ...
            <View>
              ...
              <View className="flex items-center gap-2">
                ...
                {user.openid && (
                  <View
                    className="bg-slate-800 px-3 py-1 rounded-lg active:bg-slate-700"
                    onClick={() => {
      ...
      {!user && (
        <View className="bg-red-600/20 rounded-2xl p-6 border border-red-500/30 mb-6">
          ...
          <Button
            className="w-full bg-slate-800 text-white rounded-xl py-3 font-semibold mt-3"
      ...
      {user && token && user.role !== 'admin' && (
        <View className="bg-gradient-to-br from-blue-600/20 to-purple-600/20 rounded-2xl p-6 border border-sky-500/30">
          ...
          <Text className="block text-xs text-slate-400 mt-3 text-center">
            ⚠️ 此功能仅用于开发环境
          </Text>
        </View>
      )}
      ...
      {user?.role === 'admin' && (
        <>
          ...
          <View className="space-y-3 mb-6">
            ...
            {/* 用户管理 */}
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/users/index' })}
            >
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/lexicon-manage/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>💾</Text>
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/quick-note-manage/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>✨</Text>
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-knowledge/pages/admin-knowledge-share/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>📖</Text>
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/recycle-management/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>🏪</Text>
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/dashboard/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>📈</Text>
            ...
            <View
              className="bg-slate-800 rounded-xl p-4 flex items-center justify-between border border-slate-700 active:bg-slate-800"
              onClick={() => Taro.navigateTo({ url: '/package-admin/pages/admin/audit/index' })}
            >
              <View className="flex items-center gap-3">
                <Text>📋</Text>
      ...
      {/* 提示信息 */}
      <View className="mt-8">
        <Text className="block text-xs text-slate-400 text-center leading-relaxed">
          访问管理员页面：{'\n'}
