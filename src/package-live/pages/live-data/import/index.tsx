...
const LiveDataImportPage = () => {
  ...
  const methodTabs: { key: ImportMethod; label: string; icon: React.ReactNode; desc: string }[] = [
    { key: 'manual', label: '手动填写', icon: <Text>⌨</Text>, desc: '逐条输入直播数据' },
    { key: 'excel', label: 'Excel导入', icon: <Text>📁</Text>, desc: '批量上传数据表格' },
    { key: 'screenshot', label: '截图识别', icon: <Text>📡</Text>, desc: '识别复盘截图信息' },
  ];
