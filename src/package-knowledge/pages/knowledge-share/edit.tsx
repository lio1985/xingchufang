...
const KnowledgeShareEditPage = () => {
  // 权限检查：需要员工及以上权限
  const { canAccess, loading: authLoading } = useAuthGuard({ 
    requireLogin: true,
    requiredRole: 'employee',
    forbiddenMessage: '编辑知识分享需要登录员工账号'
  });
  ...
  // 加载知识分享详情
  const loadDetail = async (shareId: string) => {
    try {
      setPageLoading(true);
      const res = await Network.request({
        url: `/api/knowledge-shares/${shareId}`,
        method: 'GET'
      });

      if (res.data?.code === 200 && res.data.data) {
        const data: KnowledgeDetail = res.data.data;
        setTitle(data.title || '');
    ...
    <View className="min-h-screen bg-slate-900">
      ...
      <View className="bg-slate-800 px-4 py-4 flex items-center justify-between border-b border-slate-700">
        ...
        <View className="flex items-center gap-2">
          <Text>📖</Text>
          <Text className="block text-lg font-bold text-white">编辑知识分享</Text>
        </View>
        <View
...
export default KnowledgeShareEditPage;
