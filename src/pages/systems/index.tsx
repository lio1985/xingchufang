...
const SystemsPage = () => {
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        <View className="flex items-center justify-between mb-3">
          <View className="flex items-center gap-2">
            ...
            {loading ? (
              <Text className="block text-xs text-slate-500">加载中...</Text>
          ...
          <View
            className="px-3 py-1.5 bg-sky-500/20 border border-sky-500/30 rounded-lg flex items-center gap-1.5 active:scale-95 transition-all"
        ...
        {inputSources?.platforms && inputSources.platforms.length > 0 && (
          <View className="mb-4">
            <Text className="block text-sm text-slate-500 mb-2">已选平台</Text>
            <View className="flex flex-wrap gap-2">
              {inputSources.platforms.map((platform: string) => (
                <View
                  key={platform}
                  className="bg-sky-500/20 border border-sky-500/30 px-3 py-1.5 rounded-lg"
                >
        ...
        {inputSources?.customQuestions && (
          <View className="mb-4">
            <Text className="block text-sm text-slate-500 mb-2">自定义问题</Text>
              ...
              {inputSources.customQuestions.liveQuestions && inputSources.customQuestions.liveQuestions.length > 0 && (
                <View className="bg-slate-50 rounded-lg p-3">
                  ...
                  {inputSources.customQuestions.liveQuestions.slice(0, 3).map((q: string, idx: number) => (
                    <Text key={idx} className="block text-xs text-slate-600 mb-1 truncate">{idx + 1}. {q}</Text>
                  ))}
                  {inputSources.customQuestions.liveQuestions.length > 3 && (
                    <Text className="block text-xs text-sky-600">还有 {inputSources.customQuestions.liveQuestions.length - 3} 个问题...</Text>
                  )}
                </View>
              )}
              {/* 销售问题 */}
              {inputSources.customQuestions.salesQuestions && inputSources.customQuestions.salesQuestions.length > 0 && (
                <View className="bg-slate-50 rounded-lg p-3">
                  ...
                  {inputSources.customQuestions.salesQuestions.length > 3 && (
                    <Text className="block text-xs text-sky-600">还有 {inputSources.customQuestions.salesQuestions.length - 3} 个问题...</Text>
                  )}
                </View>
              )}
              {/* 评论问题 */}
              {inputSources.customQuestions.commentQuestions && inputSources.customQuestions.commentQuestions.length > 0 && (
                <View className="bg-slate-50 rounded-lg p-3">
                  ...
                  {inputSources.customQuestions.commentQuestions.length > 3 && (
                    <Text className="block text-xs text-sky-600">还有 {inputSources.customQuestions.commentQuestions.length - 3} 个问题...</Text>
                  )}
                </View>
              )}
            </View>
        ...
        {inputSources?.hotTopicSources && inputSources.hotTopicSources.length > 0 && (
          <View>
            <Text className="block text-sm text-slate-500 mb-2">热点来源</Text>
            <View className="flex flex-wrap gap-2">
        ...
        {!inputSources || (
          ...
          <View className="bg-white/30 rounded-lg p-4 text-center">
            <Text className="block text-sm text-slate-500 mb-2">尚未配置输入来源</Text>
            <View
              className="inline-block px-4 py-2 bg-sky-500/20 border border-sky-500/30 rounded-lg active:scale-95 transition-all"
              onClick={() => Taro.navigateTo({ url: '/pages/input-sources/index' })}
            >
        ...
        <View className="bg-white rounded-xl border border-slate-200 p-4">
          <View className="flex items-center justify-between mb-3">
            ...
            <View className="flex items-center gap-3">
              ...
              <View className="w-px h-4 bg-slate-100"></View>
              <View className="flex items-center gap-1 text-sky-600">
                <Sparkles size={16} />
  ...
  const renderContentSystem = () => (
    <View className="flex flex-col gap-4">
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex items-center gap-2 py-3 bg-slate-50 rounded-lg px-3">
          <Text className="block text-sm text-slate-500">结构化提示词输入</Text>
          <ArrowRight size={16} color="#94a3b8" />
          <Text className="block text-sm text-sky-600 font-medium">五段式脚本输出</Text>
        </View>
      </View>
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="grid grid-cols-2 gap-2">
          <View className="bg-slate-50 rounded-lg p-3 text-center">
            <Text className="block text-sm text-white mb-1">快答版</Text>
            <Text className="block text-xs text-slate-500">30秒快速解答</Text>
          </View>
          <View className="bg-slate-50 rounded-lg p-3 text-center">
            <Text className="block text-sm text-white mb-1">清单版</Text>
            <Text className="block text-xs text-slate-500">要点清单列举</Text>
          </View>
          <View className="bg-slate-50 rounded-lg p-3 text-center">
            <Text className="block text-sm text-white mb-1">预算对比版</Text>
            <Text className="block text-xs text-slate-500">价格方案对比</Text>
          </View>
          <View className="bg-slate-50 rounded-lg p-3 text-center">
            <Text className="block text-sm text-white mb-1">避坑版</Text>
            <Text className="block text-xs text-slate-500">常见误区提醒</Text>
          </View>
          <View className="bg-slate-50 rounded-lg p-3 text-center col-span-2">
            <Text className="block text-sm text-white mb-1">案例延伸版</Text>
            <Text className="block text-xs text-slate-500">实际案例拓展</Text>
          </View>
        </View>
      </View>
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex flex-col gap-2">
          <View className="flex items-center gap-3 py-2 bg-slate-50 rounded-lg px-3">
            <Text className="block text-sm text-white">抖音</Text>
            <Text className="block text-xs text-slate-500">视频平台</Text>
          </View>
          <View className="flex items-center gap-3 py-2 bg-slate-50 rounded-lg px-3">
            <Text className="block text-sm text-white">小红书</Text>
            <Text className="block text-xs text-slate-500">图文平台</Text>
          </View>
        </View>
      </View>
    </View>
  ...
  const renderLexiconSystem = () => (
    <View className="flex flex-col gap-4">
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex flex-col gap-2">
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-emerald-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">沉淀标准术语</Text>
          </View>
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-emerald-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">行业表达</Text>
          </View>
          <View className="flex items-start gap-2 py-2">
            <Text className="block text-emerald-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">成本逻辑与判断模型</Text>
          </View>
        </View>
      </View>
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex flex-col gap-2">
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-sky-600 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">主播真实表达</Text>
          </View>
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-sky-600 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">常用判断话术</Text>
          </View>
          <View className="flex items-start gap-2 py-2">
            <Text className="block text-sky-600 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">成交表达风格</Text>
          </View>
        </View>
      </View>
  ...
  const renderViralSystem = () => (
    <View className="flex flex-col gap-4">
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex flex-col gap-2">
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            ...
            <Text className="block text-sm text-slate-600">热门内容采集</Text>
          </View>
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-red-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">成功要素提取</Text>
          </View>
          <View className="flex items-start gap-2 py-2 border-b border-slate-200">
            <Text className="block text-red-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">模板制作</Text>
          </View>
          <View className="flex items-start gap-2 py-2">
            <Text className="block text-red-400 mt-1">•</Text>
            <Text className="block text-sm text-slate-600">复刻推荐算法</Text>
          </View>
        </View>
      </View>
      ...
      <View className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <View className="flex flex-col gap-2">
          <View className="flex items-center gap-2 py-2 bg-slate-50 rounded-lg px-3">
  ...
  return (
    <View className="min-h-screen bg-sky-50">
      {/* 顶部导航 */}
      <View className="bg-white px-4 py-3 flex items-center gap-2 flex-wrap">
        <View
          className="flex items-center text-slate-500 px-3 py-1.5 rounded-lg"
        ...
        <View
          className={`flex items-center px-3 py-1.5 rounded-lg ${activeType === 'topic' ? 'text-sky-600 bg-blue-500/10' : 'text-slate-500'}`}
          onClick={() => {
            setActiveType('topic');
            loadTopicQuestions();
          }}
