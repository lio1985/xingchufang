    ...
    <View className="min-h-screen bg-sky-50">
      ...
      <View className="sticky top-0 z-50 bg-sky-50/95 backdrop-blur-md border-b border-slate-800">
        <View className="flex items-center justify-between px-4 py-3">
          ...
          >
            <Text className={`block text-sm ${submitting || !name.trim() ? 'text-slate-500' : 'text-white'}`}>
              {submitting ? '创建中...' : '创建'}
            </Text>
          </View>
        </View>
      ...
      <View className="p-4 space-y-4">
        ...
        <View className="bg-sky-50 rounded-xl p-4 border border-slate-800">
          <Text className="block text-sm text-slate-500 mb-2">团队名称 *</Text>
        ...
        {/* Team Description */}
        <View className="bg-sky-50 rounded-xl p-4 border border-slate-800">
          <Text className="block text-sm text-slate-500 mb-2">团队描述</Text>
        ...
        {/* Leader ID */}
        <View className="bg-sky-50 rounded-xl p-4 border border-slate-800">
          <Text className="block text-sm text-slate-500 mb-2">负责人ID</Text>
          <View className="bg-white rounded-lg px-3 py-2">
            <Input
              className="text-base text-white bg-transparent"
        ...
        <View className="flex items-start gap-2 p-4 bg-blue-500/10 rounded-xl border border-blue-500/20">
          <Users size={18} className="text-sky-600 flex-shrink-0 mt-0.5" />
          <View>
            <Text className="block text-sm text-sky-600 font-medium mb-1">创建说明</Text>
            <Text className="block text-xs text-slate-500">创建团队后，您可以在团队详情页添加更多成员。团队成员可以共享客户数据和回收门店信息。</Text>
          </View>
        </View>
      </View>
    </View>
