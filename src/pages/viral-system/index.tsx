    ...
    <View className="min-h-screen bg-sky-50 pb-8">
      ...
      <View className="sticky top-0 z-10 bg-sky-50/95 backdrop-blur-sm border-b border-slate-800 px-4 py-5">
        <View className="flex items-center justify-between">
          ...
          <View
            className="px-3 py-2 bg-slate-50 rounded-xl active:scale-95 transition-all"
            onClick={handleGoToFavorites}
          >
            <View className="flex items-center gap-1.5">
              <Heart size={18} color="#ef4444" />
              <Text className="text-sm text-slate-600">收藏夹</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <View className="px-4 mt-6 flex flex-col gap-6">
        ...
        {step !== 'input' && (
          <View className="bg-white/90 rounded-2xl border border-slate-200/80 p-5">
            ...
            {isProcessing && (
              <View className="mt-6 bg-white/60 rounded-2xl p-5">
                <View className="flex items-center justify-between mb-3">
                  ...
                  <Text className="text-sm text-sky-600 font-bold">
                    {Math.round(progress)}%
                  </Text>
                </View>
                ...
                <View className="mt-3 flex flex-col gap-1.5">
                  <Text className="text-xs text-slate-500 flex items-center gap-1.5">
        ...
        {step === 'input' && (
          <View className="bg-white/90 rounded-2xl border border-slate-200/80 p-5">
            ...
            <View className="bg-white/60 rounded-2xl p-4 mb-4">
              <Textarea
                className="w-full bg-transparent text-slate-600 text-base leading-relaxed"
            ...
            <Text className="block text-xs text-slate-500 mb-4">
              💡 系统将对您粘贴的文案进行爆款结构分析
            </Text>
        ...
        {step === 'completed' && currentAnalysis && (
          <>
            ...
            <View className="bg-white/90 rounded-2xl border border-slate-200/80 p-5">
              ...
              <View className="space-y-4">
                <View className="bg-white/60 rounded-xl p-4">
                  ...
                  <Text className="block text-sm text-slate-500 mt-2">{currentAnalysis.framework.description}</Text>
                </View>

                <View className="bg-white/60 rounded-xl p-4">
                  <Text className="block text-sm font-bold text-sky-600 mb-2">结构拆解</Text>
                  <View className="space-y-3">
                    <View>
                      <Text className="block text-xs font-semibold text-slate-600 mb-1">🎣 钩子（Hook）</Text>
                      <Text className="block text-sm text-slate-600">{currentAnalysis.structure.hook}</Text>
                    </View>
                    <View>
                      <Text className="block text-xs font-semibold text-slate-600 mb-1">📝 主体内容</Text>
                      {currentAnalysis.structure.body.map((item, idx) => (
                        <Text key={idx} className="block text-sm text-slate-600 mb-1">
                          {idx + 1}. {item}
                        </Text>
                      ))}
                    </View>
                    <View>
                      ...
                      <Text className="block text-sm text-slate-600">{currentAnalysis.structure.climax}</Text>
                    </View>
                    <View>
                      <Text className="block text-xs font-semibold text-slate-600 mb-1">📢 号召（CTA）</Text>
                      <Text className="block text-sm text-slate-600">{currentAnalysis.structure.callToAction}</Text>
                    </View>
                  </View>
                </View>
                ...
                <View className="bg-white/60 rounded-xl p-4">
                  ...
                  {currentAnalysis.framework.keyPoints.map((point, idx) => (
                    <View key={idx} className="flex items-start gap-2 mb-2">
                      ...
                      <Text className="block text-sm text-slate-600">{point}</Text>
                    </View>
                  ))}
                </View>
              </View>
            ...
            {/* 重新分析 */}
            <View
              className="py-3 rounded-xl bg-slate-50 active:scale-[0.98] transition-all"
              onClick={handleReset}
            >
              <View className="flex items-center justify-center gap-2">
                <RefreshCw size={16} color="#94a3b8" />
                <Text className="text-sm text-slate-600">重新分析</Text>
              </View>
            </View>
          </>
        )}
