        ...
        <>
          ...
          <View className="p-4 bg-slate-800">
            ...
            <View style={{ display: 'flex', flexDirection: 'row', gap: '8px', marginBottom: '8px' }}>
              ...
              >
                <View className="bg-slate-700 px-4 py-3 rounded-xl flex items-center justify-center">
              ...
              <View className="bg-slate-800 rounded-xl p-4 mb-6">
                {topItems.items.map((item: any, index: number) => (
                  <View key={item.id} className="flex items-center py-3 border-b border-slate-700 last:border-0" onClick={() => handleViewDetail(item.id)}>
                    <View className={`w-6 h-6 rounded-full flex items-center justify-center mr-3 ${index < 3 ? 'bg-yellow-500' : 'bg-slate-700'}`}>
