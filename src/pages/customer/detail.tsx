  ...
  const loadData = async () => {
    ...
    try {
      const [customerRes, followUpsRes] = await Promise.all([
        Network.request({ url: `/api/customers/${id}` }),
              ...
              {followUps.map((followUp, index) => (
                <View key={followUp.id} className="relative pl-6 pb-6 last:pb-0">
                  ...
                  {index < followUps.length - 1 && (
                    <View className="absolute left-2 top-3 bottom-0 w-0.5 bg-slate-700" />
                  )}
                  ...
                  <View className="bg-slate-700/50 rounded-lg p-3">
                    <View className="flex justify-between items-start mb-2">
                      ...
                      {followUp.follow_up_method && (
                        <View className="bg-slate-600 px-2 py-0.5 rounded">
                    ...
                    {followUp.next_follow_up_plan && (
                      <View className="mt-2 pt-2 border-t border-slate-600">
            ...
            <View className="mb-4">
              ...
              <View className="flex gap-2">
                {['电话', '微信', '面谈', '其他'].map((method) => (
                  <View
                    key={method}
                    className={`px-3 py-1.5 rounded-full ${followUpMethod === method ? 'bg-blue-600' : 'bg-slate-700'}`}
