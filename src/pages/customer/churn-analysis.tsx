                ...
                <View className="grid grid-cols-2 gap-3">
                  <View className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 rounded-2xl p-4 border border-sky-500/30">
                    <View className="flex items-center gap-2 mb-2">
                      ...
                      <Text className="block text-slate-600 text-xs">挽回成功率</Text>
                    </View>
                  ...
                  <View className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 rounded-2xl p-4 border border-emerald-500/30">
                    <View className="flex items-center gap-2 mb-2">
                      ...
                      <Text className="block text-slate-600 text-xs">成交转化率</Text>
                    </View>
                ...
                <View className="bg-white/50 rounded-2xl p-4 border border-slate-200">
                  ...
                  <View className="grid grid-cols-4 gap-3">
                    <View className="text-center">
                      ...
                      <Text className="block text-slate-500 text-xs mt-1">总预警</Text>
                    </View>
                    <View className="text-center">
                      <Text className="block text-sky-600 text-xl font-bold">{stats.handledCount}</Text>
                      <Text className="block text-slate-500 text-xs mt-1">已处理</Text>
                    </View>
                    <View className="text-center">
                      <Text className="block text-amber-400 text-xl font-bold">{stats.pendingCount}</Text>
                      <Text className="block text-slate-500 text-xs mt-1">待跟进</Text>
                    </View>
                    <View className="text-center">
                      <Text className="block text-slate-500 text-xl font-bold">{stats.failedCount}</Text>
                      <Text className="block text-slate-500 text-xs mt-1">挽回失败</Text>
                    </View>
                  </View>
                </View>
                            ...
                            <View key={action} className="flex items-center gap-3">
                              ...
                              <View className="flex-1">
                                <View className="flex items-center justify-between mb-1">
                                  <Text className="block text-slate-600 text-sm">{actionLabels[action] || action}</Text>
                                  <Text className="block text-slate-500 text-xs">{data.count}次</Text>
                                </View>
                                ...
                                <Text className="block text-sky-600 text-xs mt-1">成功率 {data.successRate}%</Text>
                              </View>
                            </View>
                          );
                        })}
                    ...
                    <View className="space-y-3">
                      {Object.entries(stats.byRiskLevel).map(([level, data]) => (
                        <View key={level} className="flex items-center gap-3">
                          ...
                          <View className="flex-1">
                            <View className="flex items-center justify-between mb-1">
                              <Text className="block text-slate-600 text-sm">{riskLevelLabels[level]}</Text>
                              <Text className="block text-slate-500 text-xs">{data.count}个</Text>
                            </View>
                            <View className="h-2 bg-white rounded-full overflow-hidden">
                              <View
                ...
                {stats.monthlyTrend.length > 0 ? (
                  <>
                    ...
                    <View className="bg-white/50 rounded-2xl p-4 border border-slate-200">
                        ...
                        {stats.monthlyTrend.map((month, idx) => {
                          ...
                          return (
                            <View key={idx} className="flex-1 flex flex-col items-center">
                              ...
                              <Text className="block text-slate-500 text-xs mt-2">
                                {month.month.slice(5)}
                              </Text>
                            </View>
                          );
                      ...
                      <View className="flex items-center justify-center gap-6 mt-4">
                        <View className="flex items-center gap-2">
                          ...
                          <Text className="block text-slate-500 text-xs">预警数</Text>
                        </View>
                        <View className="flex items-center gap-2">
                          <View className="w-3 h-3 bg-emerald-500/70 rounded" />
                          <Text className="block text-slate-500 text-xs">挽回数</Text>
                        </View>
                      </View>
                    </View>
                    ...
                    <View className="bg-white/50 rounded-2xl p-4 border border-slate-200">
                      ...
                      <View className="space-y-3">
                        {stats.monthlyTrend.map((month, idx) => (
                          <View key={idx} className="flex items-center justify-between py-2 border-b border-slate-200 last:border-0">
                            <View className="flex items-center gap-2">
                              <Calendar size={14} color="#64748b" />
                              <Text className="block text-slate-600 text-sm">{month.month}</Text>
                            </View>
                            <View className="flex items-center gap-4">
                              <Text className="block text-sky-600 text-sm">预警 {month.warnings}</Text>
                  ...
                  <View className="py-20 text-center">
                    ...
                    <Text className="block text-slate-500">暂无趋势数据</Text>
                  </View>
                )}
              </View>
            )}
              ...
              <View className="space-y-4">
                {rankings.length > 0 ? (
                        ...
                        >
                          ...
                          <View className="flex-1">
                            ...
                            <View className="flex items-center gap-3 mt-1">
                              <Text className="block text-slate-500 text-xs">处理 {handler.totalHandled}</Text>
                          ...
                          <View className="text-right">
                            <Text className="block text-sky-600 font-bold">{handler.successRate}%</Text>
                            <Text className="block text-slate-500 text-xs">成功率</Text>
                          </View>
                        </View>
                      ))}
                    </View>
                  ...
                  <View className="py-20 text-center">
                    ...
                    <Text className="block text-slate-500">暂无排行数据</Text>
                    <Text className="block text-slate-500 text-sm mt-2">处理预警后将显示排行榜</Text>
                  </View>
                )}
              </View>
            )}
