  ...
  return (
    <View className="min-h-screen bg-slate-900 pb-20">
      <ScrollView scrollY className="h-full">
        {/* 标题区 */}
        <View className="bg-gradient-to-br from-sky-50 via-slate-800 to-slate-900 px-6 pt-4 pb-6">
          {/* 返回按钮 */}
          <View
        ...
        <View className="px-6 py-6">
                ...
                <View className="space-y-3">
                  ...
                  <View className="flex items-center gap-3">
                    ...
                    <View className="flex-1">
                      ...
                      <View className="h-2 bg-slate-800 rounded-full overflow-hidden">
                        <View
                          className="h-full bg-slate-8000 transition-all"
              ...
              {loadingRelated ? (
                ...
                <View className="space-y-2">
                  {relatedTopics.map((topic, index) => (
                    <View
                      key={topic.id}
                      className="bg-slate-800/30 rounded-lg p-3 active:scale-[0.98] transition-transform"
                      onClick={() => {
                        Taro.navigateTo({
          ...
          {url && url !== '#' && (
            <View
              className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/30 active:scale-[0.98] transition-all cursor-pointer"
            ...
            >
              <View className="flex items-center gap-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className="block text-xs text-slate-400">复制链接到剪贴板</Text>
                </View>
              </View>
            </View>
          )}
      ...
      <View className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-lg border-t border-slate-800 px-6 py-4 safe-area-bottom">
        <View className="flex gap-3">
          ...
          <View style={{ flex: 1 }}>
            <Button
              className="w-full h-12 rounded-xl bg-blue-500 border-0 text-white font-medium flex items-center justify-center gap-2 shadow-lg shadow-blue-500/30"
              onClick={() => {
                // 复制内容到剪贴板
            ...
            >
              <Text className="text-base text-white font-medium">分享</Text>
            </Button>
          </View>
        </View>
      </View>
