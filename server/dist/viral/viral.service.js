...
let ViralService = class ViralService {
    ...
    async parseDouyinUrl(url) {
        ...
        try {
            ...
            const redirectRes = await axios_1.default.head(url, {
                maxRedirects: 5,
                timeout: 10000,
                headers: {
    ...
    async extractTextWithLLM(shareText) {
...
只返回提取到的文案内容，不要有任何解释或说明。`;
    ...
    extractTextFromShare(shareText) {
    ...
    async extractVideoAudioToText(videoUrl) {
        console.log('🎤 [viral] 开始语音识别:', videoUrl);
        if (!videoUrl) {
            throw new common_1.BadRequestException('视频URL不能为空');
        }
        try {
            ...
            throw new common_1.BadRequestException(`语音识别失败: ${error.message || '请检查视频是否包含语音或链接是否有效'}`);
        }
    }
    async extractVideo(url) {
        console.log('📥 [viral] 开始提取视频:', url);
        throw new common_1.BadRequestException('请使用 analyzeDouyinContent 方法进行完整分析');
    }
    async transcribeAudio(audioUrl) {
        console.log('🎤 [viral] 开始语音识别:', audioUrl);
        if (!audioUrl) {
            throw new common_1.BadRequestException('音频 URL 不能为空');
        }
        try {
            const result = await this.asrClient.recognize({
                uid: 'user_' + Date.now(),
            ...
            throw new common_1.BadRequestException(`语音识别失败: ${error.message || '请检查音频 URL 是否有效'}`);
        }
    }
    async transcribeAudioFromBase64(base64Audio) {
        console.log('🎤 [viral] 开始语音识别（Base64）');
        if (!base64Audio) {
            throw new common_1.BadRequestException('音频数据不能为空');
        }
        try {
            const result = await this.asrClient.recognize({
                uid: 'user_' + Date.now(),
            ...
            console.error('🎤 [viral] 语音识别失败:', error);
            throw new common_1.BadRequestException(`语音识别失败: ${error.message || '请检查音频数据是否有效'}`);
        }
    }
    async analyzeContent(transcript, platform) {
        console.log('🔍 [viral] 开始内容分析:', { platform, transcriptLength: transcript.length });
        if (!transcript || transcript.trim().length === 0) {
            throw new common_1.BadRequestException('分析内容不能为空');
        }
        try {
  ...
  "framework": {
    ...
    "keyPoints": ["关键要点1", "关键要点2", "关键要点3"]
            ...
            try {
                ...
                throw new common_1.BadRequestException('内容分析失败，AI 返回格式错误');
            }
            if (!analysisResult.structure || !analysisResult.framework) {
                throw new common_1.BadRequestException('内容分析失败，数据结构不完整');
            }
            ...
            throw new common_1.BadRequestException(`内容分析失败: ${error.message || '请稍后重试'}`);
        }
    }
    async analyzeDouyinContent(shareText) {
        console.log('📥 [viral] 开始分析抖音内容:', shareText.substring(0, 100));
        if (!shareText || shareText.trim().length === 0) {
            throw new common_1.BadRequestException('分享内容不能为空');
        }
        ...
        if (!extractedText || extractedText.trim().length === 0) {
            throw new common_1.BadRequestException('无法从分享内容中提取到文案，请:\n' +
                '1. 确保分享内容完整（包含文案）\n' +
                '2. 或直接粘贴视频文案进行分析');
        }
    ...
    async analyzeTranscript(transcript, title, desc) {
...
请严格按照以下JSON格式输出：
{
  ...
  "framework": {
    ...
    async favoriteFramework(userId, title, structure, framework) {
        console.log('❤️ [viral] 收藏爆款框架:', { userId, title });
        if (!title || !structure || !framework) {
            throw new common_1.BadRequestException('收藏数据不完整');
        }
        try {
            ...
            if (error) {
                ...
                throw new common_1.BadRequestException(`收藏失败: ${error.message}`);
            }
            ...
            throw new common_1.BadRequestException(`收藏失败: ${error.message || '请稍后重试'}`);
        }
    }
    async getFavorites(userId) {
        ...
        try {
            ...
            throw new common_1.BadRequestException(`获取失败: ${error.message || '请稍后重试'}`);
        }
    }
    async remixContent(data) {
        ...
        if (!transcript || !structure || !framework || !remixIdea) {
            throw new common_1.BadRequestException('二创改写数据不完整');
        }
        try {
            let lexiconContext = '';
...
请基于以上信息，生成2个二创方案。确保返回的是合法的JSON格式。`;
            ...
            try {
                ...
                if (!schemeA && !schemeB) {
                    throw new common_1.BadRequestException('二创内容解析失败');
                }
                parsedResponse = {
                    schemes: [
                        {
            ...
            if (!parsedResponse.schemes || !Array.isArray(parsedResponse.schemes)) {
                throw new common_1.BadRequestException('二创内容格式错误');
            }
            ...
            throw new common_1.BadRequestException(`二创改写失败: ${error.message || '请稍后重试'}`);
        }
    }
    async optimizeIdea(idea, transcript, style) {
        console.log('🪄 [viral] 开始优化改写想法:', { ideaLength: idea.length, transcriptLength: transcript?.length || 0, style });
        if (!idea || idea.trim().length === 0) {
            throw new common_1.BadRequestException('改写想法不能为空');
        }
        try {
            const styleMap = {
                douyin: '抖音',
...
请优化这个改写想法。`;
            ...
            throw new common_1.BadRequestException(`优化失败: ${error.message || '请稍后重试'}`);
        }
    }
};
exports.ViralService = ViralService;
exports.ViralService = ViralService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [database_service_1.DatabaseService])
], ViralService);
//# sourceMappingURL=viral.service.js.map