"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AiAnalysisController = void 0;
const common_1 = require("@nestjs/common");
const ai_analysis_service_1 = require("./ai-analysis.service");
let AiAnalysisController = class AiAnalysisController {
    constructor(aiAnalysisService) {
        this.aiAnalysisService = aiAnalysisService;
    }
    async analyzeQuestions(body) {
        const result = await this.aiAnalysisService.analyzeQuestions(body.questions);
        return { code: 200, msg: 'success', data: result };
    }
    async recommendTopics(body) {
        const result = await this.aiAnalysisService.recommendTopics(body.platforms, body.questions, body.hotTopics, body.preferences);
        return { code: 200, msg: 'success', data: result };
    }
};
exports.AiAnalysisController = AiAnalysisController;
__decorate([
    (0, common_1.Post)('analyze-questions'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], AiAnalysisController.prototype, "analyzeQuestions", null);
__decorate([
    (0, common_1.Post)('recommend-topics'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], AiAnalysisController.prototype, "recommendTopics", null);
exports.AiAnalysisController = AiAnalysisController = __decorate([
    (0, common_1.Controller)('ai-analysis'),
    __metadata("design:paramtypes", [ai_analysis_service_1.AiAnalysisService])
], AiAnalysisController);
//# sourceMappingURL=ai-analysis.controller.js.map