...
理性看待热点，客观分析事件。本内容由{platform}平台热榜聚合整理，仅供参考。`
   ...
   * 获取创作角度建议
   */
  async getCreativeAngles(keyword: string, category: string = ''): Promise<string[]> {
    console.log('=== 生成创作角度建议 ===');
    console.log('关键词:', keyword);
    console.log('分类:', category);

    // 基于关键词和分类生成创作角度建议
    const angles = [
      `深度解读：从专业角度分析"${keyword}"的内在逻辑和发展趋势`,
      `用户视角：探讨"${keyword}"对普通用户/消费者的实际影响`,
      `对比分析：将"${keyword}"与同类话题/竞品进行横向对比`,
      `背后故事：挖掘"${keyword}"背后的创作者/团队故事`,
      `未来展望：预测"${keyword}"的发展趋势和可能的后续走向`,
      `争议讨论：客观呈现关于"${keyword}"的不同观点和争议`,
      `实用指南：基于"${keyword}"为用户提供实用的操作建议或教程`,
      `数据解读：用数据说话，分析"${keyword}"背后的数据趋势`,
    ];

    // 根据关键词长度随机选择4-6个角度
    const numAngles = 4 + (keyword.length % 3);
    const shuffled = angles.sort(() => 0.5 - Math.random());
    return shuffled.slice(0, numAngles);
  }

  /**
   * 获取热点时间轴
   */
  async getTimeline(hotness: number, publishTime?: string): Promise<Array<{ time: string; hotness: number; event: string }>> {
    console.log('=== 生成热点时间轴 ===');
    console.log('热度:', hotness);
    console.log('发布时间:', publishTime);

    return this.generateTimeline(hotness, publishTime);
  }
