...
export class ContentWritingService {
   ...
   * 生成内容大纲
   */
  async generateOutline(dto: GenerateOutlineDto): Promise<Record<string, any>> {
...
请生成详细的内容大纲。`,
   ...
   * 扩写内容段落
   */
  async expandContent(dto: ExpandContentDto): Promise<Record<string, any>> {
    const messages = [
      {
        role: 'system' as const,
...
请扩写出这个章节的详细内容。`,
   ...
   * 润色优化内容
   */
  async polishContent(dto: PolishContentDto): Promise<Record<string, any>> {
...
请优化这段内容。`,
   ...
   * 生成完整内容
   */
  async generateFullContent(dto: GenerateFullContentDto): Promise<Record<string, any>> {
    const platformTips = this.getPlatformTips(dto.platform);
...
请生成完整的内容脚本。`,
   ...
   * 获取创作灵感
   */
  async suggestInspiration(dto: SuggestInspirationDto): Promise<Record<string, any>> {
    const messages = [
      {
        role: 'system' as const,
...
请提供详细的创作灵感建议。`,
   ...
   * 解析AI响应
   */
  private parseAIResponse(content: string): Record<string, any> {
    try {
      // 尝试提取 JSON
